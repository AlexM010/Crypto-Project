...
def set_patch_log(file_path, file_name, transition_info, severity, change, patched_file, lines, method):
    """
...
def replace_DES(weak_cipher, path, lines, language, patch_log, severity):
    """
    ...
    patch_log.append(set_patch_log(path, os.path.basename(path), transition_str, severity, f"[+] Patching {weak_cipher} » {cipher_replacement.get(weak_cipher)} in {language}", patched_file, lines, "AUTO"))
...
def replace_RC4(weak_cipher, path, lines, language, patch_log, severity):
    """
...
def replace_MD5_SHA1_SHA256_with_SHA512(weak_cipher, path, lines, language, patch_log, severity):
    """
...
def process_scan(scan, patch_log):
    ...
    for vulnerability in scan["vulnerabilities"]:
        ...
        if fix_func:
            fix_func(weak_cipher, file_path, lines, language, patch_log, severity)
        else:
